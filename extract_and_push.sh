...
PARTITIONS=(system systemex system_ext system_other
    ...
    my_custom my_manifest my_carrier my_region my_bigball my_version special_preload vendor_dlkm odm_dlkm system_dlkm
)
...
[[ -z ${codename} ]] && codename=$(grep -m1 -oP "(?<=^ro.product.vendor.device=).*" -hs /vendor/euclid/my_manifest/buildmy*.prop)
